...
- **オーバーハングの支持判定**: 下向き面が「支持されているか」の判定は、重心の真下・
  **ベッド直上**の1点標本で行う。スライサ文献の定義は「**下方から支持されていない**部分」
  であり、両者は**空隙を挟んだ天井**で乖離する——例えば 16mm の空隙を持つスロット天井は、
  ベッド高さに材料があるという理由で**見逃される**（既知の偽陰性・問302 で実測確認）。
  真下への光線で「直下に接して材料があるか」を測る修正を試みたが、ベッドすれすれ
  (0.02mm) の dome rim——**材料ではなくベッドに**支持される面——を誤検出する退行が出たため
  採用していない。正しい判定には「材料による支持」と「ベッドによる支持」を分けて扱う
  設計が要る。なお報告される**落差（支持高さ）は実測値**であり、この限界の影響を受けない。
...
- `cargo test`: 449 テスト（436 ライブラリユニット + 5 CLI ユニット + 8 統合）合格。
